...
//    @Autowired private AssessmentCategoryRepo categoryRepo;
//    @Autowired private UserRepo userRepo;
//    @Autowired private PeerSessionRequestRepo peerSessionRequestRepo;
//
//    /* The logged-in TPO admin's email, or null if not authenticated. */
...
//     */
//    private String degreeLabel(StudentRegistration s) {
//        if (s == null || s.getHasUndergraduate() == null || !s.getHasUndergraduate()) return "Unspecified";
//        String degree = s.getUndergraduateDegree();
//        if (degree == null || degree.isBlank()) return "Unspecified";
//        if ("Other".equalsIgnoreCase(degree)) {
//            String other = s.getUndergraduateOtherDegree();
...
//    //
//    // Every learner's session request first lands here as PENDING_TPO_APPROVAL.
//    // It's only visible to a TPO admin when BOTH the teacher and the learner
//    // belong to that admin's college (same "college" matching rule as
//    // scopedStudents() above: undergraduate OR postgraduate university equals
//    // the admin's college). Approving flips status to PENDING, which is what
//    // makes it show up in the teaching student's own incoming-requests list
//    // for the first time — before that, the teacher never sees it at all.
//
//    private StudentRegistration registrationFor(String email) {
//        if (email == null) return null;
//        return registrationRepo.findByEmail(email).orElse(null);
//    }
//
//    private String yearLabel(StudentRegistration s) {
//        if (s == null || s.getCurrentYear() == null || s.getCurrentYear().isBlank()) return "Unspecified";
//        return s.getCurrentYear();
//    }
//
//    /* True if the given user's undergraduate or postgraduate university
//     * matches `college` — same rule scopedStudents() uses for the students table. */
//    private boolean belongsToCollege(String email, String college) {
//        if (college == null) return false;
//        StudentRegistration s = registrationFor(email);
//        if (s == null) return false;
//        return college.equalsIgnoreCase(s.getUndergraduateUniversity())
//                || college.equalsIgnoreCase(s.getPostGraduationUniversity());
//    }
//
//    private Map<String, Object> toPeerSessionAdminMap(PeerSessionRequest s) {
//        StudentRegistration teacherReg = registrationFor(s.getTeacherEmail());
//        StudentRegistration learnerReg = registrationFor(s.getLearnerEmail());
//
//        Map<String, Object> m = new LinkedHashMap<>();
//        m.put("id", s.getId());
...
//        m.put("tpoApproved", s.getTpoApproved());
//        m.put("tpoAdminEmail", s.getTpoAdminEmail());
//        m.put("tpoRespondedAt", s.getTpoRespondedAt() != null ? s.getTpoRespondedAt().toString() : null);
//        m.put("createdAt", s.getCreatedAt() != null ? s.getCreatedAt().toString() : null);
//
//        Map<String, Object> teacher = new LinkedHashMap<>();
//        teacher.put("email", s.getTeacherEmail());
//        teacher.put("name", s.getTeacherName());
//        teacher.put("degree", degreeLabel(teacherReg));
//        teacher.put("year", yearLabel(teacherReg));
//        m.put("teacher", teacher);
//
//        Map<String, Object> learner = new LinkedHashMap<>();
//        learner.put("email", s.getLearnerEmail());
//        learner.put("name", s.getLearnerName());
//        learner.put("degree", degreeLabel(learnerReg));
//        learner.put("year", yearLabel(learnerReg));
//        m.put("learner", learner);
//
//        return m;
//    }
//
//    /* "Session Requests" tab — awaiting this TPO's decision. */
//    public List<Map<String, Object>> getPendingPeerSessionRequests() {
//        String college = currentAdminCollege();
//        if (college == null) return List.of();
//        return peerSessionRequestRepo.findByStatusOrderByCreatedAtDesc("PENDING_TPO_APPROVAL").stream()
//                .filter(s -> belongsToCollege(s.getTeacherEmail(), college) && belongsToCollege(s.getLearnerEmail(), college))
//                .map(this::toPeerSessionAdminMap)
//                .collect(Collectors.toList());
//    }
//
//    /* "Teacher Student Requests" tab — this TPO's approval history/tracking. */
//    public List<Map<String, Object>> getApprovedPeerSessionRequests() {
//        String college = currentAdminCollege();
//        if (college == null) return List.of();
//        return peerSessionRequestRepo.findByTpoApprovedOrderByTpoRespondedAtDesc(Boolean.TRUE).stream()
//                .filter(s -> belongsToCollege(s.getTeacherEmail(), college) && belongsToCollege(s.getLearnerEmail(), college))
//                .map(this::toPeerSessionAdminMap)
//                .collect(Collectors.toList());
//    }
//
//    public Map<String, Object> respondToPeerSessionRequest(Long id, boolean approve) {
//        PeerSessionRequest session = peerSessionRequestRepo.findById(id)
//                .orElseThrow(() -> new NoSuchElementException("Session request not found"));
//
//        String college = currentAdminCollege();
//        if (college == null || !belongsToCollege(session.getTeacherEmail(), college)
//                || !belongsToCollege(session.getLearnerEmail(), college)) {
//            throw new RuntimeException("This session request isn't visible to your college");
//        }
//        if (!"PENDING_TPO_APPROVAL".equals(session.getStatus())) {
//            throw new RuntimeException("This request has already been reviewed");
//        }
//
//        session.setTpoApproved(approve);
//        session.setTpoAdminEmail(currentAdminEmail());
//        session.setTpoRespondedAt(LocalDateTime.now());
//        session.setStatus(approve ? "PENDING" : "REJECTED_BY_TPO");
//
//        peerSessionRequestRepo.save(session);
//        return toPeerSessionAdminMap(session);
//    }
//}
...
public class TpoService {
    ...
    @Autowired private UserRepo userRepo;
    @Autowired private PeerSessionRequestRepo peerSessionRequestRepo;
    @Autowired private PeerTopicRepo peerTopicRepo;
    ...
    private Map<String, Object> toPeerSessionAdminMap(PeerSessionRequest s) {
        ...
        /* Topic detail (description / level / tags / availability) lives on PeerTopic,
         * not on the request row, so pull it in for the TPO review panel. */
        PeerTopic topic = (s.getTopicId() == null)
                ? null
                : peerTopicRepo.findById(s.getTopicId()).orElse(null);

        Map<String, Object> topicInfo = new LinkedHashMap<>();
        topicInfo.put("description", topic != null ? topic.getDescription() : null);
        topicInfo.put("level", topic != null ? topic.getLevel() : null);
        topicInfo.put("sessionDuration", topic != null ? topic.getSessionDuration() : null);
        topicInfo.put("timeSlot", topic != null ? timeSlotLabel(topic.getTimeSlotStart(), topic.getTimeSlotEnd()) : null);
        topicInfo.put("tags", (topic != null && topic.getTags() != null) ? topic.getTags() : List.of());
        topicInfo.put("availableDays", (topic != null && topic.getAvailableDays() != null) ? topic.getAvailableDays() : List.of());
        m.put("topic", topicInfo);
    ...
    /* "10:00 AM - 11:00 AM", or just the start if no end is stored. */
    private String timeSlotLabel(String start, String end) {
    ...
    /* Requests this TPO has already rejected — feeds the "Rejected" stat + filter. */
    public List<Map<String, Object>> getRejectedPeerSessionRequests() {
        String college = currentAdminCollege();
        if (college == null) return List.of();
        return peerSessionRequestRepo.findByTpoApprovedOrderByTpoRespondedAtDesc(Boolean.FALSE).stream()
                .filter(s -> belongsToCollege(s.getTeacherEmail(), college) && belongsToCollege(s.getLearnerEmail(), college))
                .map(this::toPeerSessionAdminMap)
                .collect(Collectors.toList());
    }
